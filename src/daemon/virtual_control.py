...
from handGesture_recognition import HandGesture_recognition  # noqa: E402
...
class VirtualControl(HandGesture_recognition):
    ...
    def is_HiFive_gesture(self, hand_landmarks: List):
        handLandmarks_forOneHand = hand_landmarks[
            0
        ]  # use only 1 hand for detect

        def comparing_Yvalue(list_indexes: tuple):
            if len(list_indexes) == 4:
                a, b, c, d = list_indexes
                return (
                    handLandmarks_forOneHand[a][1]
                    > handLandmarks_forOneHand[b][1]
                    > handLandmarks_forOneHand[c][1]
                    > handLandmarks_forOneHand[d][1]
                )
    ...
    def is_click_action(self, hand_landmarks: List) -> bool:
        def checking_validDeque(sequence_deque: Deque) -> bool:
            if len(sequence_deque) > 2:  # for prevent out of index
                if sequence_deque[-1] == [0, 0] and not sequence_deque[-2] == [
                    0,
                    0,
                ]:
                    return True
        ...
        if self.is_HiFive_gesture(hand_landmarks=hand_landmarks):
            ...
            # check valid deque
            if checking_validDeque(
                sequence_deque=self.point_history[first_key]
            ):
                return True
    ...
    def clicking_mouse(
        self, width_ratio: float, height_ratio: float, hand_landmarks: List
    ):

        if self.is_click_action(hand_landmarks=hand_landmarks):
            first_key = next(iter(self.point_history))
            if (
                len(self.point_history[first_key]) == 0
            ):  # for prevent out of index
                return

            pointX, pointY = self.point_history[first_key][
                self.history_length - 2
            ]  # before hi-five gestures
    ...
    def moving_mouse(self, width_ratio: float, height_ratio: float):
        first_key = next(iter(self.point_history))
    ...
    def run(self):
        ...
        while not self.stop_thread:
            ...
            key = (
                cv.waitKey(1)
                if not self.use_static_image_mode
                else cv.waitKey(0)
                if image is not None and self.use_static_image_mode
                else cv.waitKey(1)
            )
            if key == 27:  # ESC
                break
            number, mode, auto, prev_number = self._select_mode(
                key, mode, auto, prev_number
            )
            ...
            image = (
                image if self.disable_image_flip else cv.flip(image, 1)
            )  # mirror display
            ...
            (
                ...
                not_rotate_rects,
            ) = self._palm_detection(image=image, debug_image=debug_image)

            hand_landmarks = self._hand_landmark(
                cropted_rotated_hands_images=cropted_rotated_hands_images,
                rects=rects,
            ...
            debug_image = self._draw_point_history(
            ...
            if hand_landmarks is not None and len(hand_landmarks) > 0:
                ...
                is_hiFiveGesture_happened = self.is_HiFive_gesture(
                ...
                if self.using_mouse:
                    if len(self.point_history) > 0:  # key-point gesture
                        if (
                            not is_hiFiveGesture_happened
                        ):  # prevent mouse move when use hiFiveGesture
                            self.moving_mouse(
                        ...
                        if (
                            not previous_hiFiveGesture_status
                            and is_hiFiveGesture_happened
