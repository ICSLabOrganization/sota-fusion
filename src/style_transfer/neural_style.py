...
class NeuralStyle:
    ...
    def _load_modelPath(self, modelName: str) -> list:
        valid_files = [
            f for f in self.MODEL_PATH.glob("*") if f.stem == modelName
        ]
    ...
    def __call__(
        self, args_content_image: Path, args_model: str, args_cuda: bool
    ):
        ...
        args_model = str(
            self._load_modelPath(modelName=args_model)[0]
        )  # first index
    ...
    def stylize_onnx(self, content_image, model):
        ...
        ort_inputs = {
            ort_session.get_inputs()[0].name: to_numpy(content_image)
        }
