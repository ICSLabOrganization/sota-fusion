...
def get_moveOver_event(ID: int, event=None):
    if ID == 0:
        if "Enter" in event:
            canvas.itemconfig(
                button_generate_canvas, image=img_btn_generate_enabled
            )

        elif "Leave" in event:
            canvas.itemconfig(button_generate_canvas, image=img_btn_generate)
        ...
        if "Enter" in event:
            canvas.itemconfig(btn_delete_canvas, image=img_btn_delete_enabled)

        elif "Leave" in event:
            canvas.itemconfig(btn_delete_canvas, image=img_btn_delete)
...
img_btn_generate_enabled = PhotoImage(
    file=relative_to_assets("btn_generate_enabled.png")
)
img_btn_delete_enabled = PhotoImage(
    file=relative_to_assets("btn_delete_enabled.png")
)
...
button_generate_canvas = canvas.create_image(
    615.0, 442.0, image=img_btn_generate
)

image_result_canvas = canvas.create_image(618.0, 215.0, image=img_result)

btn_record_canvas = canvas.create_image(208.0, 102.0, image=img_btn_record)

btn_delete_canvas = canvas.create_image(332.0, 215.0, image=img_btn_delete)
########################static gui##############################
canvas.tag_bind(
    button_generate_canvas,
    "<Button-1>",
